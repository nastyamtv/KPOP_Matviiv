...
export function showAddTeacherPopUp() {
    ...
    addButton.onclick = function () {
        ...
        let isBirthDateValid = birthDate !== undefined && birthDate !== null && String(birthDate).length > 0;
        let isEmailValid = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(email);
        let isPhoneValid = /^[\+]?[(]?[0-9]{3}[)]?[-\s\.]?[0-9]{3}[-\s\.]?[0-9]{4,6}$/.test(phoneNumber);

        if (name.length < 3 || city.length < 2 || !isEmailValid || !isPhoneValid || !isBirthDateValid || notes.length < 3 || ((genderMale === null || !genderMale.checked) && (genderFemale === null || !genderFemale.checked))) {
