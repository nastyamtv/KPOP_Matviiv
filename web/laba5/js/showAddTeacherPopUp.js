import { hideAddTeacherPopUp } from "./hideAddTeacherPopIp.js";
import { loadAndDisplayStatistics } from "./loadAndDisplayStatistics.js";
import { globalFilteredTeachers, teachers } from "./loadUsers.js";
import { saveTeacherToDB } from "./saveTeacherToDB.js";
...
export function showAddTeacherPopUp() {
        ...
        <div class="popup-content add-teacher-popup-content">
            ...
            <form class="add-teacher-form">
                ...
                <div class="form-row-field">
                    <input
                        class="add-teacher-form-input add-teacher-form-input-margin-top add-teacher-form-input-margin-bottom"
                        type="date" name="birth-date" id="birth-date" placeholder="MM/DD/YYYY">
                </div>
                ...
                <div class="form-row add-teacher-form-input-margin-bottom">
                    ...
                    <label for="background-color"><img src="images/color-picker.png" alt="Color picker"></label>
    ...
    addButton.onclick = function() {
        ...
        let isPhoneValid = /^[\+]?[(]?[0-9]{3}[)]?[-\s\.]?[0-9]{3}[-\s\.]?[0-9]{4,6}$/.test(phoneNumber);

        if (name.length < 3 || city.length < 2 || !isEmailValid || !isPhoneValid || notes.length < 3 || ((genderMale === null || !genderMale.checked) && (genderFemale === null || !genderFemale.checked))) {
        ...
        saveTeacherToDB(teacher);
        
        hideAddTeacherPopUp();
        updateTopteachersGrid();
        loadAndDisplayStatistics(globalFilteredTeachers, 1);
    };

    let closeAddTeacherButton = document.getElementById('close-add-teacher-button');
